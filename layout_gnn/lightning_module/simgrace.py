...
class SimGRACE(LightningModule):
    ...
    def update_perturbed_encoder(self) -> None:
...
class SimGRACEEncDec(SimGRACE):
    def __init__(
        ...
        projection_head: nn.Module, 
        perturbation_magnitude: float = 1., 
        decoder_loss_weight: float = 1., 
        loss_fn: Optional[nn.Module] = None, 
        lr: float = 0.01    
    ) -> None:
        super().__init__(encoder, projection_head, perturbation_magnitude, loss_fn, lr)
        self.decoder = decoder
        self.decoder_loss_weight = decoder_loss_weight
        self.decoder_loss = nn.MSELoss(reduction='sum')
    ...
    def forward_loss(
        ...
        **kwargs
        ...
        z, z_ = self(inputs["graph"], update_perturbed_encoder=update_perturbed_encoder)
        loss = self.loss_fn(z, z_)
...
class LayoutGNNSimGRACERasterizer(SimGRACEEncDec):
    """Extends `SimGRACE` to instantiate the encoder and projection head."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        Args:
            ...
            cnn_hidden_dim (int, optional): Hidden dimension of the decoder CNN. Defaults to 16.
            cnn_output_size (int, optional): Size of the decoded image. Defaults to 64.
            projection_head_dims (Optional[Union[int, Sequence[int]]]): Layer sises of the projection head MLP.
            perturbation_magnitude (float): Magnitude of the perturbations for the perturbed branch in SimGRACE.
            loss_fn (Optional[nn.Module]): Loss function used to train the encoder. If not provided, NTXent is used.
        ...
        super().__init__(
            ...
            perturbation_magnitude=perturbation_magnitude,
            loss_fn=loss_fn,
            decoder_loss_weight=reconstruction_loss_weight,
            lr=lr,
        )
        
        
class LayoutGNNSimGRACERasterizerNTXent(LayoutGNNSimGRACERasterizer):
    def __init__(
        self, 
        num_labels: int, 
        label_embedding_dim: int = 64, 
...
class LayoutGNNSimGRACERasterizerVICReg(LayoutGNNSimGRACERasterizer):
    def __init__(
        self, 
        num_labels: int, 
        label_embedding_dim: int = 64, 
